...
export function ensureTool(tool) {
    try {
...
export function scaffoldFramework(stack, targetDir, name) {
    ...
    try {
        if (stack === 'flutter') {
            ...
            execSync(`cd ${parentDir} && npx create-next-app@latest ${dirName} --use-npm --typescript --eslint --no-src-dir --app --import-alias "@/*"`, { stdio: 'inherit' });
             ...
             console.log(kleur.cyan('🏗️  Constructing Python FastAPI Service...'));
             
             // 1. Requirements.txt
             const requirements = `fastapi>=0.109.0\nuvicorn>=0.27.0\npython-dotenv>=1.0.1\npydantic>=2.6.1\n`;
             fs.writeFileSync(path.join(targetDir, 'requirements.txt'), requirements);
...
async def root():
    return {
        "status": "active",
        "system": "Shadow Gravity Python Backend",
        "version": "0.1.0"
    }
