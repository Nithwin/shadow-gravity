...
export function generateMission(stack, features, customDescription) {
  let rules = [];
  ...
  return `# 🛡️ MISSION PROTOCOL
> **SYSTEM ROLE:** You are acting as a Senior Software Architect.

---

## 📋 PHASE 0: Context & Planning
1.  **Analyze:** Read \`package.json\` and \`.antigravity/CURRENT_TASK.md\`.
2.  **Memory:** Read \`.antigravity/DECISIONS.md\` to understand past architectural choices.
3.  **Plan:** Create or update \`.antigravity/CURRENT_PLAN.md\` before executing code.
...
## ⛔ CONSTRAINTS
${negativeConstraints || "1. Do not invent import paths; use existing project structure."}

---

## 🏁 PHASE 4: Verification
1.  Run tests if available.
2.  Run compliance: \`npm run agent:verify\` to ensure clean code.
3.  Output: "✅ BUILD SUCCESSFUL" when complete.
`;
}

export function injectShadow(targetDir, stack, type) {
    ...
    pkg.devDependencies["fs-extra"] = "^11.1.1";
